...
#define WAMR_PROC(exec_env)                                                    \
    ((wasm_process_t *)wasm_runtime_get_user_data(exec_env))
...
static void wasm_api_print(wasm_exec_env_t exec_env, const char *ptr,
                           uint32_t len)
{
...
static int32_t wasm_api_read_line(wasm_exec_env_t exec_env, char *buf,
                                  int32_t max_len)
{
    wasm_process_t *proc = WAMR_PROC(exec_env);
    tty_t *tty = tty_get(proc->tty_id);
...
static int32_t wasm_api_read(wasm_exec_env_t exec_env, int32_t fd, uint8_t *buf,
                             int32_t count)
{
    wasm_process_t *proc = WAMR_PROC(exec_env);
...
static int32_t wasm_api_mkdir(wasm_exec_env_t exec_env, const char *path,
                              int32_t path_len)
{
    (void)exec_env;
    char pathbuf[256];
...
static int32_t wasm_api_unlink(wasm_exec_env_t exec_env, const char *path,
                               int32_t path_len)
{
    (void)exec_env;
    char pathbuf[256];
...
static int32_t wasm_api_rmdir(wasm_exec_env_t exec_env, const char *path,
                              int32_t path_len)
{
    (void)exec_env;
    char pathbuf[256];
                               ...
                               int32_t pid, int32_t addr, int32_t data)
{
    ...
    case PTRACE_GETREGS: {
        ...
        ptrace_info_t *info =
            wasm_runtime_addr_app_to_native(inst, (uint64_t)data);
        *info = e->ptrace_info;
        return 0;
    }
    default:
...
static int32_t wasm_api_dup2(wasm_exec_env_t exec_env, int32_t oldfd,
                             int32_t newfd)
{
    wasm_process_t *proc = WAMR_PROC(exec_env);
    if (oldfd < 0 || oldfd >= WASM_MAX_FDS || newfd < 0 ||
        newfd >= WASM_MAX_FDS) {
        return -1;
    }
...
static int32_t wasm_api_pipe_close_read(wasm_exec_env_t exec_env,
                                        int32_t pipe_id)
{
    (void)exec_env;
    pipe_unref_read(pipe_id);
...
static int32_t wasm_api_pipe_close_write(wasm_exec_env_t exec_env,
                                         int32_t pipe_id)
{
    (void)exec_env;
    pipe_unref_write(pipe_id);
...
static int32_t wasm_api_spawn_redirected(wasm_exec_env_t exec_env,
                                         ...
                                         int32_t argc, const uint8_t *redir_buf,
                                         int32_t redir_len)
{
    char pathbuf[256];
    int copy_len = path_len < 255 ? path_len : 255;
    ...
    wasm_process_t *proc = WAMR_PROC(exec_env);
    return wasm_spawn_redirected(pathbuf, actual_argc, argv, proc->pid, setups,
                                 setup_count);
}
...
static uint32_t wasm_api_fb_alloc(wasm_exec_env_t exec_env, uint32_t w,
                                  uint32_t h)
{
...
static void wasm_api_fb_flush(wasm_exec_env_t exec_env, uint32_t src_offset,
                              uint32_t dst_x, uint32_t dst_y, uint32_t w,
                              uint32_t h, uint32_t src_pitch)
{
